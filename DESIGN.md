...
### ログ一覧画面 (`/dashboard` または `/`) ※認証後

-   **コンポーネント:** `app/page.tsx` (現状は仮実装)
-   **機能:**
    -   ログインユーザーの今日のログを取得し、表示する。
    -   ログは `app/page.tsx` 内の `getTodaysLogs` 関数で取得される。
    -   取得したログは `LogEntry` インターフェースに従って表示される。
    -   認証機能が未実装のため、現在は `user_id` がハードコーディングされている。
-   **データ取得:** `lib/redis.ts` の `redisClient` を使用して、Redisからログデータを取得する。
-   **表示ロジック:**
    -   `LogEntry` の配列を受け取り、タイムスタンプ (`ts`) とアプリケーション名 (`app`) を表示する。
